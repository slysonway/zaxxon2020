...
Game::Game()
	: mWindow(sf::VideoMode(80 * BLOCK_COUNT, 600), "Space Invaders 1978", sf::Style::Close)
	, mTexture()
	, mPlayer()
	, mFont()
	, mStatisticsText()
	...
	, mIsMovingLeft(false)
{
	...
	_TextureBlock.loadFromFile("Media/Textures/SI_WALL_BLOCK.png");
	_TextureBackGround.loadFromFile("Media/Textures/background.jpg");
...
void Game::InitSprites()
{
	...
	{
		int x = rand() % SPRITE_COUNT_X;
		sf::Sprite enemy;
		enemy.setTexture(_TextureEnemy);
		enemy.setPosition(100.f + 50.f * x, 100.f * (i + 1));
		std::shared_ptr<Entity> se = std::make_shared<Entity>();
		se->m_sprite = enemy;
		se->m_type = EntityType::enemy;
		se->m_size = _TextureEnemy.getSize();
		se->m_position = enemy.getPosition();
	...
	int randomClearWall = rand() % BLOCK_COUNT;

	for (int i = 0; i < BLOCK_COUNT; i++)
	{
		if (i == randomClearWall)
		{
			continue;
		}
		_Block[i].setTexture(_TextureBlock);
		_Block[i].setPosition(80.f * i, 10); // default position

		//_Block[i].setPosition(0.f + 150.f * (i + 1), 10.f + 10.f);
	...
	// Boss Number
	//
	_BossNumberText.setFillColor(sf::Color::Green);
	_BossNumberText.setFont(mFont);
	_BossNumberText.setPosition(10.f, 150.f);
	_BossNumberText.setCharacterSize(20);
	_BossNumberText.setString(std::to_string(_bossNumber));
...
void Game::HandleTexts()
{
	...
	std::string bossNumber = "Boss: " + std::to_string(_bossNumber);
	_BossNumberText.setString(bossNumber);
	return;
}
...
// use to handle big enemy move left right
...
// use to handle when enemy shoot in block 
...
// use to handle when user get shot by ennemies
...
// use to handle when enemiry fire is out of bound or mv fire
...
//use to handle when enemy has to fire 
...
//use to handle and move enemies right left
...
// use to handle when shoot go out of bounds to reset weapon fire of user
...
// use to handle when user it a block 
...
// use to handle when user hit enemy with weapon
...
void Game::HandleCollisionBlockPlayer() 
{
	for (std::shared_ptr<Entity> block : EntityManager::m_Entities)
	{
		if (block->m_type != EntityType::block)
		...
		if (boundBlock.intersects(boundPlayer) == true)
		{
			ResetPlayer();
			_lives--;
			goto end;
...
void Game::ResetPlayer()
{
	...
	EntityManager::GetPlayer()->m_sprite.setPosition(mPlayer.getPosition());
}

void Game::HandleBlockMoves()
{
	for (std::shared_ptr<Entity> entity : EntityManager::m_Entities)
	{
		if (entity->m_enabled == false)
...
void Game::HandleBlockDisabled()
{
	int randomClearWall = rand() % (BLOCK_COUNT - 1);
	int index = 0;
	for (std::shared_ptr<Entity> entity : EntityManager::m_Entities)
	{
		if (entity->m_type != EntityType::block)
		...
		if (index >= randomClearWall)
		{
			entity->m_sprite.setPosition(80.f * (index + 1), 10);
		}
		else
		{
			entity->m_sprite.setPosition(80.f * index, 10);
		}
		index++;
	}
}
...
void Game::HandleEnemyDisabled()
{
	for (std::shared_ptr<Entity> entity : EntityManager::m_Entities)
	{
		...
		float y = entity->m_sprite.getPosition().y;
		y = y - 600;

		entity->m_sprite.setPosition(100.f + 50.f * x, y);
	}
}

void Game::HandleMasterSpawn()
{

	if (EntityManager::GetEnemyMaster()->m_enabled == true)
	{
		return;
	}

	if (_score > (_scoreBossSpawn * (_bossNumber + 1)))
	{
